...
function Menu() {
  ...
  // ✅ LIKES - ஒரே ஒரு API call (egress fix)
  const [likedIds, setLikedIds] = useState([]);

  const user = JSON.parse(localStorage.getItem("user"));

  /* ================================
     🟢 FETCH PRODUCTS - Once only
  ================================ */
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`${API}/api/products/`);
  ...
  /* ================================
     ✅ FETCH LIKES - ஒரே ஒரு call
     20 product cards la தனித்தனியா
     call பண்ணாம இங்க மட்டும் fetch
  ================================ */
  useEffect(() => {
    if (!user) return;

    axios.get(`${API}/api/likes/`, {
  ...
  // filter/search change ஆனா page 1 reset
  useEffect(() => {
    setCurrentPage(1);
  }, [search, filter]);
  ...
  const suggestions = useMemo(() => {
    const safeProducts = Array.isArray(products) ? products : [];
    if (!cart || cart.length === 0) return safeProducts.slice(0, 3);
    const categories = cart.map(item => item.category);
    return safeProducts.filter(p => categories.includes(p.category));
  }, [products, cart]);
              ...
              {currentProducts.length > 0 ? (
                currentProducts.map(item => (
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                  >
                    {/* ✅ likedIds + setLikedIds pass பண்ணுது */}
                    <ProductCard
                      product={item}
                      likedIds={likedIds}
