...
#         if diff < 60:
#             return Response({
#                 "error": f"Wait {int(60 - diff)} seconds ⏳"
#             }, status=400)
...
#         email=email,
#         otp_type="forgot_password",
#         last_sent_at__date=today   # 🔥 FIX (use last_sent_at instead of created_at)
#     ).count()

#     if total_sent_today >= 5:
#         return Response({
#             "error": "Too many OTP requests today ❌"
#         }, status=400)
...
#     # ===== ITEMS TABLE =====
#     table_header = [['#', 'Item Name', 'Qty', 'Unit Price', 'Total']]
...
#             item.product.name,
#             str(item.quantity),
#             f"₹{float(item.product.price):.2f}",
#             f"₹{item_total:.2f}"
#         ])

#     table_data = table_header + item_rows
