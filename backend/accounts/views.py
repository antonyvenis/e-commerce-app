...
def generate_invoice(request, order_id):
    ...
    # ✅ IST TIME FIX - no pytz needed
    ist = ZoneInfo("Asia/Kolkata")
    created_ist = order.created_at.astimezone(ist)
    formatted_time = created_ist.strftime('%d %b %Y %I:%M %p')

    # ✅ TODAY DATE
    today_str = datetime.date.today().strftime('%d %b %Y')

    # ✅ INVOICE NUMBER - separate variable (date hide fix)
    ...
    header_data = [[
        ...
        Paragraph(
            f"<b><font size=18 color='#333333'>INVOICE</font></b><br/><br/>"
            f"<font size=9 color='grey'>Invoice No: <b>{invoice_no}</b><br/>"
            f"Date: <b>{today_str}</b></font>",
            styles['Normal']
        )
    ]]
    ...
    info_data = [[
        Paragraph(
            f"<b><font size=10 color='#ff6600'>BILL TO</font></b><br/>"
    ...
    items_table = Table(
        table_data,
        colWidths=[0.4*inch, 3*inch, 0.7*inch, 1.2*inch, 1.2*inch]
    )
