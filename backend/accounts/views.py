...
from .serializers import RegisterSerializer
from .models import CustomUser, Like, CartItem, Order, OrderItem, OTP, Product
...
def forgot_password_send_otp(request):
    ...
    if not CustomUser.objects.filter(email=email, username=username).exists():
        return Response({"error": "Invalid username/email ❌"}, status=400)
...
def reset_password(request):
    ...
    if not password:
        return Response({"error": "Enter password ❌"}, status=400)
    ...
    try:
        ...
        return Response({"error": "Verify OTP ❌"}, status=400)
